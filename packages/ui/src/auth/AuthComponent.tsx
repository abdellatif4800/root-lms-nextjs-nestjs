...
export function AuthComponent({ isPublic }: { isPublic: boolean }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-surface-950/80 backdrop-blur-sm">
      {/* Modal Container */}
      <div
        className="relative bg-surface-900 w-full max-w-md border border-surface-800 flex flex-col overflow-hidden shadow-[0_0_50px_rgba(0,0,0,0.5)]"
      ...
      >
        ...
        {/* --- Header --- */}
        <div className="h-12 border-b border-surface-800 bg-surface-950/50 flex items-center justify-between px-4 shrink-0 relative z-20">
          <div className="flex items-center gap-2">
            <span className={`w-2 h-2 rounded-full ${isLogin ? 'bg-teal-glow shadow-[0_0_8px_var(--teal-glow)]' : 'bg-purple-glow shadow-[0_0_8px_var(--purple-glow)]'} animate-pulse`} />
            <span className="font-digital text-sm tracking-widest text-text-primary">
          ...
          {isPublic || !isRequired &&
            ...
            <button
              ...
              className="text-text-secondary hover:text-red-500 font-mono text-xs transition-colors"
            >
              [ X ]
            </button>}
        </div>
        ...
        {/* --- Tabs (Login / Register) --- */}
        <div className="flex border-b border-surface-800 relative z-20">
          <button
            onClick={() => setIsLogin(true)}
            className={`flex-1 py-3 text-xs font-black uppercase tracking-widest transition-all ${isLogin
        ...
        {/* --- Form Body --- */}
        <div className="p-8 relative z-20 flex-1 bg-surface-900/50">
          {isLogin ? (
            <LoginForm />
          ) : (
            <RegisterForm />
